...
describe("EnrollmentService org-suspension gating", () => {
  describe("enrollFree", () => {
    ...
    it("allows enrollment for an org-private course when the org is active", async () => {
      const upsertEnrollment = vi.fn().mockResolvedValue(undefined);
      const service = makeService({
        findCourseAccess: vi.fn().mockResolvedValue({
          basePriceCents: 0,
          status: "PUBLISHED",
      ...
      await service.enrollFree(userId, courseId);
      expect(upsertEnrollment).toHaveBeenCalledWith(userId, courseId, undefined);
    });
  });

  describe("assertLessonAccessible", () => {
    function lessonContext(org: { status: string; accessLocksAt: Date | null } | null) {
      return {
        id: lessonId,
        section: {
          courseId,
    ...
    it("blocks lesson access once the suspension lock has taken effect", async () => {
      const service = makeService({
        findLessonAccessContext: vi
          .fn()
          .mockResolvedValue(
            lessonContext({ status: "SUSPENDED", accessLocksAt: new Date(Date.now() - 1000) }),
          ),
    ...
    it("still allows lesson access during an unexpired grace period", async () => {
      const service = makeService({
        findLessonAccessContext: vi.fn().mockResolvedValue(
          lessonContext({
            status: "SUSPENDED",
            accessLocksAt: new Date(Date.now() + 86_400_000),
          }),
        ),
        findIdByUserAndCourse: vi.fn().mockResolvedValue({ id: "enroll_1" }),
    ...
    it("is unaffected for a public course with no org", async () => {
      const service = makeService({
        findLessonAccessContext: vi.fn().mockResolvedValue(lessonContext(null)),
        findIdByUserAndCourse: vi.fn().mockResolvedValue({ id: "enroll_1" }),
        findCompletedLessonIds: vi.fn().mockResolvedValue([]),
      });
