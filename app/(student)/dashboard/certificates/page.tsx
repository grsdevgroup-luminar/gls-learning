...
import { Award, Lock, Download, GraduationCap, CheckCircle2 } from "lucide-react";
    ...
    <div className="space-y-8 p-6 md:p-8">
      <div>
        ...
        <p className="text-muted-foreground">
          You've earned {earned.length} certificate{earned.length !== 1 && "s"}. Finish a course to unlock more.
        </p>
      </div>
              ...
              <DialogTrigger render={<Card className="cursor-pointer p-0 transition-all hover:-translate-y-1 hover:shadow-lg" />}>
                <CardContent className="p-0">
                  ...
                  <div className="flex items-center justify-between p-3">
                    ...
                    <Badge variant="secondary" className="text-success"><Award className="mr-1 h-3 w-3" /></Badge>
                  </div>
                </CardContent>
              </DialogTrigger>
            ...
            {inProgress.map(({ course, pct }) => (
              <Card key={course.id}>
                <CardContent className="flex flex-col items-center gap-2 py-8 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">{pct}% complete — finish to earn your certificate</p>
                  <Button size="sm" variant="outline" className="mt-1" render={<Link href={`/learn/${course.slug}`} />}>
                    Continue
                  </Button>
                </CardContent>
              </Card>
      ...
      {earned.length === 0 && inProgress.length === 0 && (
        <Card><CardContent className="py-16 text-center text-muted-foreground">No courses yet.</CardContent></Card>
      )}
    </div>
  );
}

function CertificatePreview({ course, small }: { course: Course; small?: boolean }) {
  const instructor = getInstructor(course.instructorId);
  return (
    <div className={`relative overflow-hidden ${small ? "rounded-t-xl" : "rounded-xl"} border bg-gradient-to-br from-primary/5 via-background to-chart-2/5 ${small ? "p-5" : "p-10"} text-center`}>
      ...
      <div className="relative">
        ...
        {!small && <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Certificate of Completion</p>}
        <p className={`mt-2 ${small ? "text-xs" : "text-sm"} text-muted-foreground`}>This certifies that</p>
        <p className={`font-bold ${small ? "text-base" : "text-2xl"}`}>{demoStudent.name}</p>
        <p className={`mt-1 ${small ? "text-xs" : "text-sm"} text-muted-foreground`}>has successfully completed</p>
